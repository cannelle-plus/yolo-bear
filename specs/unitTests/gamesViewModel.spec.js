...
describe("when joining a game", function() {
    ...
    //    reactive.observable('serverPlayerAddedToGame')
    //            .subscribe(to(function(){
    //                 done();         
    //            }));
      ...
      // logger.setLevel('DEBUG');
       ...
       reactive.observable('gameAdded')
               ...
               .subscribe(to(function(x){
                  if (x==2)  
                  {
                    reactive.publish("emitFakeSocketGameCreated", { id: uuid.v1() });
                  }
                  if (x==3)
                  {
                    done();
      ...
      // logger.setLevel('DEBUG');
       ...
       reactive.observable('gameAdded')
               ...
               .subscribe(to(function(x){
                  if (x==2)  
                  {
                    reactive.publish("emitFakeSocketGameJoined", { id: '4a82199e-7c30-4a95-b194-6d40127fbb89' });
                  }
               }));
    ...
    calljsDom("when a  player abandons the game through the socket,  then the ui is updated", "games.html",  function(done, window, reactive) {
       ...
       reactive.observable('gameAdded')
               ...
               .subscribe(to(function(x){
                  if (x==2)  
                  {
                    reactive.publish("emitFakeSocketGameAbandonned", { id: '4a82199e-7c30-4a95-b194-6d40127fbb89', username : 'yoann' });
                  }
               }));
